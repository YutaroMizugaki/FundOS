...
contract TreasuryVault is ReentrancyGuard, Pausable {
    ...
    function donatePrincipal(uint256 amount, bytes32 donorRef) external nonReentrant {
    ...
    function fundGrantBudget(uint256 amount, bytes32 sourceRef) external nonReentrant {
        if (amount == 0) revert ZeroAmount();
        _transferIn(msg.sender, amount);
    ...
    function accountingSurplus() external view returns (uint256) {
